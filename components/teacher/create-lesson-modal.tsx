...
export default function CreateLessonModal({ classId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(data?.error || "Không thể tải học sinh.");
        }

        setStudents(data.students ?? []);
        setSelected((data.students ?? []).map((s: Student) => s.id));
      } catch (err) {
        setError(
  ...
  async function uploadFile(lessonId: string, file: File) {
    const prepare = await fetch("/api/learning/upload-url", {
      method: "POST",
    ...
    if (!prepare.ok) {
      throw new Error(prepared?.error || "Không thể chuẩn bị upload.");
    }
    ...
    const metadata = await fetch("/api/learning/attachments", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        kind: "lesson",
    ...
    if (!metadata.ok) {
      throw new Error(
        metadataData?.error || "Không thể lưu thông tin file."
      );
    }
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/teacher/lessons/create", {
        method: "POST",
        headers: {
      ...
      for (const file of files) {
        await uploadFile(data.lesson.id, file);
      }
          ...
          >
            <div className="room-modal-header">
              <div>
                ...
                <h3>Tạo bài học</h3>
                <p>
                  Tạo bài học và chỉ định học sinh nhận.
                </p>
              </div>
